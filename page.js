      ...
      <div className="slide-up" style={{ width:"100%", maxWidth:390 }}>
        ...
        <div style={{ background:"#fff", borderRadius:22, padding:"28px 24px", boxShadow:"0 8px 32px rgba(0,0,0,0.07)", border:"1px solid #f3f4f6" }}>
          <div style={{ display:"flex", gap:4, background:"#f3f4f6", borderRadius:12, padding:4, marginBottom:22 }}>
            <Link href="/login" style={{ flex:1, textAlign:"center", padding:"8px 0", borderRadius:9, color:"#9ca3af", fontWeight:500, fontSize:13, textDecoration:"none" }}>Entrar</Link>
            <span style={{ flex:1, textAlign:"center", padding:"8px 0", borderRadius:9, background:"#fff", color:"#111827", fontWeight:700, fontSize:13, boxShadow:"0 1px 4px rgba(0,0,0,0.09)" }}>Cadastrar</span>
          </div>

          <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <Field label="Nome completo" type="text"     value={name}     onChange={e => setName(e.target.value)}     placeholder="Ana Souza"          required />
            <Field label="E-mail"        type="email"    value={email}    onChange={e => setEmail(e.target.value)}    placeholder="você@email.com"      required />
            <Field label="Senha"         type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="mínimo 6 caracteres" required minLength={6} />
          ...
          <div style={{ textAlign:"center", marginTop:14 }}>
            <Link href="/login" style={{ color:"#9ca3af", fontSize:12 }}>Já tem conta? Entre aqui</Link>
          </div>
        </div>
      </div>
    </div>
